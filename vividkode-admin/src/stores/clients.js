...
const INITIAL_CLIENTS = [
  { id: 1, name: 'Sarah Johnson', company: 'StartupIO', email: 'sarah@startup.io', phone: '+254 711 234 567', service: 'Web Development', status: 'new', notes: 'Wants complete redesign, 20 pages', value: 4500, createdAt: '2024-04-12' },
  { id: 2, name: 'Marcus Chen', company: 'Enterprise Co', email: 'm.chen@enterprise.co', phone: '+1 415 555 0100', service: 'Data Solutions', status: 'converted', notes: 'Analytics dashboard, Salesforce integration', value: 12000, createdAt: '2024-03-15' },
...
export const useClientsStore = defineStore('clients', () => {
  ...
  const activeClients = computed(() => clients.value.filter(c => c.status === 'converted').length)
  const newLeads = computed(() => clients.value.filter(c => c.status === 'new').length)

  async function addClient(data) {
    await new Promise(r => setTimeout(r, 400))
    clients.value.unshift({ ...data, id: Date.now(), createdAt: new Date().toISOString().split('T')[0] })
  }
  ...
  return { clients, totalClients, activeClients, newLeads, addClient, updateClient, deleteClient, updateStatus }
})
